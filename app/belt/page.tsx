...
import { Crown, Award, Sparkles, TrendingUp, Shield } from "lucide-react"
...
export default function BeltPage() {
  ...
  const handleMobileFiltersChange = (newFilters: FilterState) => {
    setFilters({ ...newFilters, categories: ['belts'] })
  }
  ...
  const applyMobileFilters = () => {
    const filtered = beltProducts.filter((product) => {
  ...
  useEffect(() => {
    ...
    const newUrl = params.toString() ? `?${params.toString()}` : "/belt"
    router.replace(newUrl, { scroll: false })
  }, [filters, sortBy, currentPage, router])

  const beltProducts = mockProducts.filter(product => 
    product.name.toLowerCase().includes('belt') || 
    product.category?.toLowerCase() === 'belts'
  )

  const filteredProducts = beltProducts.filter((product) => {
  ...
  if (isMobile) {
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        {/* Mobile Hero Section */}
        <div className="bg-gradient-to-r from-amber-400 via-orange-500 to-amber-600 text-white py-12">
          <div className="px-4 text-center">
            <h1 className="text-2xl font-bold mb-2">Premium Leather Belts</h1>
            <p className="text-sm">Complete your sophisticated look with luxury belts</p>
          </div>
        </div>
        ...
        <div className="px-4 py-4">
          <div className="bg-white rounded-xl p-4 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-medium text-gray-700">Belt Collection</h3>
              <button 
            ...
            <div className="flex space-x-3 overflow-x-auto scrollbar-hide">
              {beltProducts.slice(0, 5).map((product) => (
                <div 
                  key={product.id} 
        ...
        <div className="px-4 pb-4">
          <div className="bg-white rounded-xl p-4 shadow-sm">
            <h3 className="text-sm font-medium text-gray-700 mb-4">All Belts</h3>
          ...
          <div className="fixed inset-0 z-50 bg-black bg-opacity-50">
            <div className="fixed inset-y-0 right-0 w-full max-w-sm bg-white shadow-xl overflow-y-auto">
              <div className="flex items-center justify-between p-4 border-b">
                <h2 className="text-lg font-semibold">Belt Filters</h2>
                <button 
        ...
        {isResultsOpen && (
          <div className="fixed inset-0 z-50 bg-white">
            ...
            <div className="sticky top-0 bg-white border-b p-4 flex items-center justify-between shadow-sm">
              <div>
                <h2 className="text-lg font-bold">Filtered Belts</h2>
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {mobileFilteredProducts.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-medium text-gray-900 mb-2">No belts found</h3>
                  <p className="text-gray-500">Try adjusting your filters to find more belts</p>
                  <button 
                    onClick={() => {
                      setFilters({
                        brands: [],
                      ...
                      setMobileFilteredProducts(beltProducts)
                    }}
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="flex pt-8 mt-4">
        ...
        <div className={`flex-1 pr-4 transition-all duration-700 delay-500 ${
          ...
          {/* Luxury Belt Hero Section */}
          <div className="relative overflow-hidden bg-gradient-to-br from-amber-400 via-orange-500 to-amber-600 text-white">
            {/* Animated Background Elements */}
            <div className="absolute inset-0">
              <div className="absolute top-6 left-6 w-16 h-16 bg-white/10 rounded-full animate-pulse" style={{ animationDelay: '0s', animationDuration: '4s' }}></div>
              <div className="absolute top-8 right-12 w-12 h-12 bg-white/15 rounded-lg rotate-45 animate-bounce" style={{ animationDelay: '1s', animationDuration: '3s' }}></div>
              <div className="absolute bottom-8 left-1/4 w-14 h-14 bg-white/10 rounded-full animate-pulse" style={{ animationDelay: '2s', animationDuration: '5s' }}></div>
              
              {/* Floating Belt/Luxury Icons */}
              ...
              <div className={`flex items-center justify-between mt-8 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                <div className="max-w-lg mt-4">
                  ...
                  <div className="inline-flex items-center space-x-2 mb-4 bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full">
                    ...
                    <span className="text-xs font-medium">Luxury Collection</span>
                  </div>
                  ...
                  <p className="text-white/90 mb-6 text-base">Complete your sophisticated look with our handcrafted luxury belts.</p>
                ...
                <div className="hidden md:block mt-4 relative">
                  ...
                  <div className="flex gap-4 mb-4">
                    {[
                      { icon: Crown, label: 'Luxury', value: '100%', color: 'text-yellow-300' },
                      { icon: Award, label: 'Premium', value: '5⭐', color: 'text-orange-200' },
                      { icon: Shield, label: 'Quality', value: 'A+', color: 'text-amber-200' }
                    ].map((stat, index) => {
                      const Icon = stat.icon
                      return (
                        <div 
                          key={index}
                          className={`bg-white/10 backdrop-blur-sm rounded-lg p-3 transition-all duration-300 hover:scale-110 hover:bg-white/20 cursor-pointer ${
                            hoveredStat === index ? 'shadow-2xl rotate-3' : 'hover:shadow-lg'
                          }`}
                          onMouseEnter={() => setHoveredStat(index)}
                          onMouseLeave={() => setHoveredStat(null)}
                        >
                          <Icon className={`w-5 h-5 ${stat.color} mb-2 mx-auto ${hoveredStat === index ? 'animate-spin' : ''}`} style={{ animationDuration: hoveredStat === index ? '2s' : undefined }} />
                          <div className="text-lg font-bold mb-1 text-center">{stat.value}</div>
                          <div className="text-xs text-amber-100 text-center">{stat.label}</div>
                        </div>
                      )
                    })}
                  </div>
                  ...
                  <div className="text-center">
                    <div className="text-5xl opacity-80 animate-pulse" style={{ animationDuration: '3s' }}>🔗</div>
                    <div className="text-xs text-amber-100 mt-2">Handcrafted Excellence</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="container mx-auto px-4 py-8">
            ...
            <div className="text-center mb-6">
              <p className="text-sm text-muted-foreground">
                Showing {Math.min(startIndex + 1, sortedProducts.length)} - {Math.min(startIndex + itemsPerPage, sortedProducts.length)} of {sortedProducts.length} luxury belts
              </p>
            </div>
